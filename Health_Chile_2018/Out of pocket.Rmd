...
Health_Out %>%
  ...
  geom_segment(aes(x = Country, xend = Country, y = 0, yend = `Out of pocket%ofcurrent2018`), color = ifelse(Health_Out$Country %in% c("Chile"), "springgreen4", "gray58"), size = ifelse(Health_Out$Country %in% c("Chile"), 1.1, 0.5)) +
  ...
  geom_text(aes(label = Health_Out$`Out of pocket%ofcurrent2018`), hjust = -.3, nudge_x = 0, color = "gray46", size = 3.1) +
